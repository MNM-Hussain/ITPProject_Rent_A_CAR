				...
				<div class="card" style="height: 1000px;">
					...
					<table border="1" cellpadding="12" class="table table-hover">
						...
						<tbody>
							<tr>
								...
								<td><%=rates.getCategory()%></td>
								<td><%=rates.getAmount()%></td>



								<td><a
									href="update_Rates.jsp?RateId=<%=rates.getRateId()%>"><button
											style="background-color: #8080ff;" class="btn btn-success ">Update</button></a></td>

								<td><form method="POST"
										action="DeleteRateServlet?RateId=<%=rates.getRateId()%>">
										<button style="background-color: #8080ff;"
					...
					<form class="form-container" action="AddRateServlet" method="post"
						...
						<label>category</label> <input type="text" class="form-control"
							id="exampleInputFName" placeholder="Vechile Id" name="vecid"
							required> <br>
							
							 <label>Amount</label> <input
							type="number" min="0"  step="any" class="form-control" id="exampleInputPassword1"
							placeholder="Amount" name="amount" required><br> <input
							type="submit" class="btn btn-success " value="ADD"
							style="margin-left: 500px; background-color: #8080ff;"><br>
